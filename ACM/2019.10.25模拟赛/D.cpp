...
void ball()
{
	...
	switch(nouter)
	{
		...
		case 2:
			res.x=(outer[0].x+outer[1].x)/2.0;
			res.y=(outer[0].y+outer[1].y)/2.0;
			res.z=(outer[0].z+outer[1].z)/2.0;
		...
		case 4:
			...
			for(i=0;i<3;++i)
				for(j=0;j<3;j++)
					m[i][j]=dot(q[i],q[j])*2;
			det=m[0][0]*m[1][1]*m[2][2]+m[0][1]*m[1][2]*m[2][0]+m[0][2]*m[2][1]*m[1][0]-m[0][2]*m[1][1]*m[2][0]-m[0][1]*m[1][0]*m[2][2]-m[0][0]*m[2][1]*m[1][2];
			if(fabs(det)<eps) return ;
			for(j=0;j<3;++j)
			{
				for(i=0;i<3;++i)
					m[i][j]=sol[i];
				L[j]=(m[0][0]*m[1][1]*m[2][2]+m[0][1]*m[1][2]*m[2][0]+m[0][2]*m[2][1]*m[1][0]-m[0][2]*m[1][1]*m[2][0]-m[0][1]*m[1][0]*m[2][2]-m[0][0]*m[2][1]*m[1][2])/det;
				for(i=0;i<3;++i)
					m[i][j]=dot(q[i],q[j])*2;
			}
			res=outer[0];
